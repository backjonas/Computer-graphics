...
vector<Vertex> loadUserGeneratedModel() {
	...
	for(auto i = 0u; i < faces; ++i)	{
        ...
        pos2x = FW::cos(angle_increment * (i + 1)) * radius;
        pos2z = FW::sin(angle_increment * (i + 1)) * radius;

        v0.position = 0;
        v1.position = Vec3f(pos1x, -height, pos1z);
        v2.position = Vec3f(pos2x, -height, pos2z);

        Vec3f cross_product = v2.position.cross(v1.position);
        Vec3f face_normal = cross_product.normalized(1);

        v0.normal = v1.normal = v2.normal = face_normal;
...
void App::render() {
	...
	Mat4f modelToWorld;
    modelToWorld = Mat4f::translate(camera_translation);
	...
	common_ctrl_.message(sprintf("Use PgUp, PdDn and arrowkeys to move camera, Home/End to rotate camera."), "instructions");
