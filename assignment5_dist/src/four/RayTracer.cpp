...
Vec3f mirrorDirection(const Vec3f& normal, const Vec3f& incoming) {
	...
	const auto v = incoming - 2.0f * (incoming.dot(normal) * normal);
	return v;
}
...
Vec3f RayTracer::traceRay(Ray& ray, float tmin, int bounces, float refr_index, Hit& hit, FW::Vec3f debug_color) const {
	...
	Vec3f answer = Vec3f(scene_.getAmbientLight() * m -> diffuse_color(point));
	...
	for (int i = 0; i < scene_.getNumLights(); i++) {
		...
		if (args_.shadows) {
			Ray r = Ray(point + EPSILON * dir, dir);
			Hit h = Hit();
			if (!scene_.getGroup()->intersect(r, h, tmin)) {
	...
	if (bounces >= 1) {
		...
		if (m->reflective_color(point).length() > 0.0f) {
			...
			const auto dir = mirrorDirection(normal, ray.direction);
			Ray r = Ray(point, dir);
			Hit h = Hit();
			answer += hit.material->reflective_color(point) * traceRay(r, EPSILON, bounces - 1, refr_index, h, debug_color);
		}
