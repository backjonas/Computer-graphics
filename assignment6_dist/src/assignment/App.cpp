		...
		while (std::getline(file, line))
		{
			...
			else if (keyword == "light")
			{
				...
				while (std::getline(file, line))
				{
					...
					{
						stream >> light.m_color.x >> light.m_color.y >> light.m_color.z;
					}
				}
			}
		}
...
Image App::convertBumpToObjectSpaceNormal( const Image& bump, MeshBase* pMeshIn, float alpha )
{
	...
	for ( int m = 0; m < pMesh->numSubmeshes(); ++m )
		for ( int t = 0; t < pMesh->indices(m).getSize(); ++t )
		{
			...
			bbmin = Vec2i( min(v_[0], v_[2], v_[4]) >> 8, min(v_[1], v_[3], v_[5]) >> 8 );
			// maximum: have to round up
			bbmax = Vec2i( (max(v_[0], v_[2], v_[4]) & 0xFF) ? 1 : 0, (max(v_[1], v_[3], v_[5]) & 0xFF) ? 1 : 0 );
			bbmax += Vec2i( max(v_[0], v_[2], v_[4]) >> 8, max(v_[1], v_[3], v_[5]) >> 8 );
